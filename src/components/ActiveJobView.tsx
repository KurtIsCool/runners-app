...
const ActiveJobView = ({ job, userId, onUpdateStatus, userProfile, onClose, onRateUser, onCancel, onRefresh }: { job: Request, userId: string, onUpdateStatus: (id: string, status: RequestStatus) => void, userProfile: UserProfile, onClose: () => void, onRateUser?: (req: Request) => void, onCancel?: (id: string, reason: string) => void, onRefresh?: () => void }) => {
    ...
    const handlePhotoUpload = async (url: string) => {
        ...
        if (error) {
            ...
            else if (uploadModalType === 'receipt') setReceiptPhotoUrl(null);
            else if (uploadModalType === 'delivery') setProofUrl(null);
        }
    };
             ...
             <div className="relative z-10 mt-4">
               <div className="flex justify-between items-center mb-1">
                   <span className="bg-blue-800 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-widest animate-pulse">
                     {['pending_runner', 'awaiting_payment', 'payment_review'].includes(job.status) ? 'Application Status' : 'Active Mission'}
                   </span>
