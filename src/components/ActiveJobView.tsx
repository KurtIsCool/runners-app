      ...
      <div className="fixed inset-0 bg-gray-100 z-[60] flex flex-col md:flex-row pop-in">
        ...
        <div className="w-full md:w-1/2 lg:w-1/3 bg-white flex flex-col border-r shadow-xl z-10 md:h-full order-1 h-[45vh]">
          ...
          <div className="p-4 border-t bg-white pb-safe-nav shrink-0">
             ...
             {job.status === 'delivering' && <button disabled={updating} onClick={() => handleStatusUpdate('completed')} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold text-base hover:bg-green-700 flex items-center justify-center gap-2 btn-press shadow-lg shadow-green-200">{updating ? <Loader2 className="animate-spin"/> : <>Mark Completed <CheckCircle size={18}/></>}</button>}
          </div>
        </div>
